...
<form  class="w3-container w3-center " action="{$conf->action_url}materialy"><br/>
	
		<input class="w3-input w3-black w3-round-large w3-animate-input w3-center " type="text" " placeholder="nazwa" name="nazwa" value="{$searchForm->nazwa}" /><br />
...
{strip}
    ...
    <td>{$p["dlugosc"]}</td>
		...
		<td>
			<a class="  w3-button w3-black w3-tiny w3-round-large" href="{$conf->action_url}materialEdit/{$p['id']}">Edytuj</a>
			&nbsp;
			<a class="w3-button w3-black w3-tiny w3-round-large" href="{$conf->action_url}materialDelete/{$p['id']}">Usuń</a>
		</td>
	</tr>
{/strip}
{/foreach}
...
<div class="messages bottom-margin">
	...
	{strip}
		<li class= {if $msg->isError()}error{/if} {if $msg->isWarning()}warning{/if} {if $msg->isInfo()}info{/if}">{$msg->text}</li>
	{/strip}
	{/foreach}
	</ul>
</div>
